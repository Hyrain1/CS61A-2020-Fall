...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        while i < len(opponent.deck.cards):
            if opponent.deck.cards[i].attack==other_card.attack and\
            opponent.deck.cards[i].defense==other_card.defense:
                opponent.deck.cards.pop(i)
            else:
                i+=1
        for cd in player.deck.cards:
            cd.attack += other_card.attack
            cd.defense += other_card.defense
